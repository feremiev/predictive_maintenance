...
def collapsible_section(
    ...
    """
    Render a section header that remains visible while its body is collapsed.
    """
    return dbc.Card(
        [
            dbc.CardHeader(
            ...
            dbc.Collapse(
                ...
                id=f"{section_key}-section-collapse",
                is_open=DEFAULT_SECTION_STATE[
                    section_key
                ],
            ),
        ],
            ...
            [
                ...
                dcc.Dropdown(
                    ...
                    options=[
                        ...
                        {
                            "label": "Asymmetric Huber",
                            "value": "asymmetric_huber",
                        },
                        {
                ...
                html.Div(
                    [
                        dbc.Alert(
                            (
                                "Positive prediction errors mean the model "
                                "overestimates the remaining life. "
                                "Asymmetric Huber applies a larger weight "
                                "to those dangerous late predictions."
                            ),
                            color="info",
                            className="small mt-3 mb-2",
                        ),
                        dbc.Row(
                            [
                                dbc.Col(
...
experiment_setup_body = html.Div(
    [
        dbc.Row(
            [
                dbc.Col(
...
layout = html.Div(
    [
        ...
        dcc.Store(
            id="development-section-state",
            data=DEFAULT_SECTION_STATE,
        ),
        sidebar,
        html.Main(
            [
...
# =====================================================================
# Collapsible development sections
# =====================================================================
...
def update_development_section_state(
    _expand_all_clicks,
    _collapse_all_clicks,
    _setup_clicks,
    _development_clicks,
    _official_clicks,
    _saved_clicks,
    ...
    if triggered_id == "expand-all-sections":
        return {
            key: True
            for key in SECTION_NAMES
        }

    if triggered_id == "collapse-all-sections":
        return {
            key: False
            for key in SECTION_NAMES
        }
    ...
    section_by_button = {
        "toggle-setup-section": "setup",
        "toggle-development-section": (
            "development"
        ),
        "toggle-official-section": "official",
        "toggle-saved-section": "saved",
        "toggle-data-section": "data",
    }

    if triggered_id in section_by_button:
        section_key = section_by_button[
            triggered_id
        ]
        state[section_key] = not bool(
            state[section_key]
        )
        return state

    # A successful training run updates this store with the newly
...
@callback(
    ...
    Output(
        "development-section-collapse",
        "is_open",
    ),
    Output(
        "official-section-collapse",
        "is_open",
    ),
    Output(
        "saved-section-collapse",
        "is_open",
    ),
    Output(
...
def render_development_section_state(
    section_state,
):
    state = {
        **DEFAULT_SECTION_STATE,
...
def toggle_asymmetric_huber_settings(
    selected_loss: str,
):
    if selected_loss == "asymmetric_huber":
        ...
        config.update(
            {
                ...
                "asymmetric_huber_late_weight": float(
                    state_values[
                        "asymmetric_huber_late_weight"
                    ]
                    if state_values[
        ...
        config.update(
            {
                ...
                "patience": None,
                "loss": None,
                "asymmetric_huber_late_weight": None,
                "asymmetric_huber_delta": None,
